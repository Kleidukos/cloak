...
choiceOf :: HashMap ActionID (Action RoomID) -- ^ The actions of the room
         -> ActionID                -- ^ The selected action (by the user)
         -> Either ActionError (Action RoomID) -- ^ Either the action does not exist (faulty input)
...
getActions :: (MonadState GameState m) => m (HashMap ActionID (Action RoomID))
getActions = do
